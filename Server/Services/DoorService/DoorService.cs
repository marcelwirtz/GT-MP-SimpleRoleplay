...
namespace SimpleRoleplay.Server.Services.DoorService
{
	class DoorService 
	: Script
	{
		...
		public static void LoadAllDoorsFromDB()
		{
			...
			DataTable result = DatabaseHandler.ExecutePreparedStatement("SELECT * FROM doors", parameters);
			if (result.Rows.Count != 0)
			{
				...
				API.shared.consoleOutput(LogCat.Info, result.Rows.Count + " Doors Loaded..");
			}
			else
			{
				API.shared.consoleOutput(LogCat.Info, "No Doors Loaded..");
			}
		}
		...
		public static void AddNewDoor(int model, Vector3 position)
		{
			...
			DataTable result = DatabaseHandler.ExecutePreparedStatement("INSERT INTO doors (Hash, Position, Locked, State) " +
