...
var BarPosX = Math.round(resX); // 320
...
API.onPlayerEnterVehicle.connect(function (veh, targetSeat) {
	if (targetSeat == -1) { isVehicleDriver = true; }
	isInVehicle = true;
});
	...
	if (API.hasEntitySyncedData(API.getLocalPlayer(), "hud")) {
		if (API.getEntitySyncedData(API.getLocalPlayer(), "hud")) {
			...
			if (API.hasEntitySyncedData(API.getLocalPlayer(), "hunger")) {
				...
				API.drawRectangle(BarPosX, BarPosY + 20, BarBGWidth, BarBGHeight, 0, 0, 0, 100); // Hunger BG
				API.drawRectangle(BarPosX + 3, BarPosY + 20 + ((BarHeight / 100) * (100 - Hunger)) + 3, BarWidth, (BarHeight / 100) * Hunger, 255, 202, 96, 150); // Hunger Bar
				if (fadedIn) {
			...
			if (API.hasEntitySyncedData(API.getLocalPlayer(), "thirst")) {
				...
				API.drawRectangle(BarPosX + 22, BarPosY + 20, BarBGWidth, BarBGHeight, 0, 0, 0, 100); // Thirst BG
				API.drawRectangle(BarPosX + 25, BarPosY + 20 + ((BarHeight / 100) * (100 - Thirst)) + 3, BarWidth, (BarHeight / 100) * Thirst, 0, 222, 255, 150); // Thirst Bar
				if (fadedIn) {
			...
			if (isVehicleDriver) {
				...
				if (vhealth > 1000) { vhealth = 1000; }
				if (vhealth < 0) { vhealth = 0; }
				API.drawRectangle(BarPosX + 44, BarPosY + 20, BarBGWidth, BarBGHeight, 0, 0, 0, 100); // Vehicle Health BG
				API.drawRectangle(BarPosX + 47, BarPosY + 20 + ((BarHeight / 1000) * (1000 - vhealth)) + 3, BarWidth, (BarHeight / 1000) * vhealth, 145, 249, 152, 150); // Vehicle Health Bar
				if (fadedIn) {
				...
				if (API.hasEntitySyncedData(veh, "fuel") && API.hasEntitySyncedData(veh, "maxfuel")) {
					...
					if (vfuel > vmaxfuel) { vfuel = vmaxfuel; }
					if (vfuel < 0) { vfuel = 0; }
					API.drawRectangle(BarPosX + 66, BarPosY + 20, BarBGWidth, BarBGHeight, 0, 0, 0, 100); // Vehicle Fuel BG
					if (vfuel <= 5) {
						if (BlinkStatus) {
							API.drawRectangle(BarPosX + 69, BarPosY + 20 + ((BarHeight / vmaxfuel) * (vmaxfuel - vfuel)) + 3, BarWidth, (BarHeight / vmaxfuel) * vfuel, 255, 0, 0, 150); // Vehicle Fuel Bar
						} else {
							API.drawRectangle(BarPosX + 69, BarPosY + 20 + ((BarHeight / vmaxfuel) * (vmaxfuel - vfuel)) + 3, BarWidth, (BarHeight / vmaxfuel) * vfuel, 255, 243, 110, 150); // Vehicle Fuel Bar
						}
					} else {
						API.drawRectangle(BarPosX + 69, BarPosY + 20 + ((BarHeight / vmaxfuel) * (vmaxfuel - vfuel)) + 3, BarWidth, (BarHeight / vmaxfuel) * vfuel, 255, 243, 110, 150); // Vehicle Fuel Bar
					}
			...
			/* ==== Compass by 3marproof (https://github.com/3marproof/Compass) ==== */
